...
export const secretHuntNumber = number => String(number).padStart(3, '0');

function validateRecordings(levels, assetExists, {idFor, allowEmpty = false}) {
  if (!Array.isArray(levels) || (!allowEmpty && !levels.length)) throw new Error('Add at least one recording');
  const ids = new Set();
  ...
  for (const level of levels) {
    ...
    if (level.id !== idFor(level.number) || ids.has(level.id)) throw new Error('Recording ID must match its number');
...
export function validateLevels(levels, assetExists = () => true) {
  validateRecordings(levels, assetExists, {idFor: number => `level-${levelNumber(number)}`});
}

export function validateSecretHunts(hunts, assetExists = () => true) {
  validateRecordings(hunts, assetExists, {idFor: number => `secret-hunt-${secretHuntNumber(number)}`, allowEmpty: true});
}

export function catalogueMeta(levels, secretHunts = []) {
  validateLevels(levels);
  validateSecretHunts(secretHunts);
  const latest = [...levels, ...secretHunts].reduce((date, level) => level.recorded > date ? level.recorded : date, levels[0].recorded);
  return {
    count: levels.length,
    secretHuntCount: secretHunts.length,
    latest,
