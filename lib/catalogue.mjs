...
export function validateLevels(levels, assetExists = () => true) {
  ...
  for (const level of levels) {
    ...
    for (const [field, pattern] of [
      ['file', /^(?:media\/|\/baba-is-you-media\/media\/)[\w-][\w.-]*\.mp4$/],
...
// Resolve the one approved companion store for local build verification.
export function localAssetURL(path, root) {
  if (/^\/baba-is-you-media\/media\/[\w-][\w.-]*\.mp4$/.test(path)) return new URL(`..${path}`, root);
