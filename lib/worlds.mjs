...
export function renderRecordingGroup(world, {open = false, labelFor = () => ''} = {}) {
  return `<details class="world-group" data-world${open ? ' open' : ''}>
...
<ol class="level-list">${world.levels.map(l => `<li id="${l.id}" data-level="${l.id}" data-number="${l.number}"${labelFor(l) ? ` data-label="${escape(labelFor(l))}"` : ''} data-title="${escape(l.title)}" data-poster="${l.poster}" data-seconds="${l.seconds}">
...
export function renderWorlds(levels) {
  return groupLevels(levels).map((world, index) => renderRecordingGroup(world, {open: index === 0})).join('\n');
}
