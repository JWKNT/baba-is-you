...
## Current checkpoint
- 40 completed in fresh Slot3: base00–07; Lake01–13 and Extras1–2; Island00–11 and Extras1,2,4,5; Ruins01. Two flowers. Slot2 untouched.
- 35 published and verified live at commit83f9bcb, Actions34095630487. Batch8 (36–40) is being prepared; do not call it published until verified.
- Catalogue36–39 edited with full Approach/Mechanics/Attempts notes. 040 Fragility encoding. No active game recorder after040 stop.
- Parked Island Extra3 Tiny Isle: recordings/tiny-isle-part1.*; Island Extra6 Evaporating River: recordings/evaporating-river-part1.* and part2.*. No completions. Reentering resets puzzles. Preserve all parts and use join_attempts.py after eventual completion; never discard them.
- New tools/pair_path.py routes multiple YOU on manually observed grids, avoiding all movable-word pushes. tools/join_attempts.py joins retained attempts and rebases input timestamps; originals remain untouched.
...
- Input: tools/input, CGEvent script explicitly authorized; U/D/L/R/E(return)/S(space)/Z(undo)/N(restart)/B(escape). Quote sequences; pass recordings/NNN.jsonl during capture. Wait for a running input session before sending more input.
- Start: python3 tools/session.py start N. Stop aftervictory: python3 tools/session.py stop. Edit with tools/edit.py, catalogue with tools/catalogue.py, fill notes, verify_batch.py, node build.mjs, tests, browser checks, explicit git staging, push main, verify Actions and public media.
- Do not POST redundant Pages builds; git push triggers Actions. Live verification uses curl because Python urllib host certificates fail. Source /Users/jw/Desktop/bin/baba-is-you, JWKNT/baba-is-you, https://jehlp.net/baba-is-you/.
- No walkthroughs, old-run solutions, or game internals consulted. All mechanics and puzzle plans from observed game state and reasoning.
- Continue actively through at least2026-09-07 09:30America/New_York. Current~05:08. Heartbeat continue-baba-is-you-playthrough every5min resumes if idle. Caffeinate active. Do not stop after a batch.
