...
test('rebuild is deterministic and every recording has a native link',()=>{
 ...
 for(const l of levels){assert.ok(after.includes(`href="${l.file}" data-watch`));assert.ok(after.includes(`id="${l.id}"`));assert.ok(statSync(new URL(l.file,root)).size>1000);}
});
...
test('all level identities are distinct and media files fit the repository limit',()=>{
 ...
 for(const l of levels)assert.ok(statSync(new URL(l.file,root)).size<100*1024*1024);
});
